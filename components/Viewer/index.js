...
export default function Viewer({ images, handle }) {
    ...
    const init = searchParams.get("img") ?? 0;
    ...
    return (
        <div className="w-full" {...bindWheel()} {...bindMove()}>
            ...
            <div className="flex lg:flex-col w-full lg:w-auto justify-between lg:justify-start items-start absolute pr-[40px] lg:pr-0 left-[20px] bottom-[20px] lg:bottom-[80px]">
                ...
                <Link className="hidden lg:block border-b border-b-[#000] mb-5" href="/">artists</Link>
                <div className="flex">
                    <div className="cursor-pointer" onClick={() => setSelected(selected === images.length - 1 ? 0 : selected + 1)}>
                        <LeftArrow width={20} />
                    </div>
                    <div className="ml-10 cursor-pointer" onClick={() => setSelected(selected === 0 ? images.length - 1 : selected - 1)}>
                        <RightArrow width={20} />
                    </div>
                </div>
            </div>
